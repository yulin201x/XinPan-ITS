...
chcp 65001 >nul
title XinPan-ITS 启动器

echo ========================================
echo   XinPan-ITS 绿色版启动器
echo ========================================
echo.
...
chcp 65001 >nul
title XinPan-ITS 停止器

echo ========================================
echo   XinPan-ITS 绿色版停止器
echo ========================================
echo.
...
; XinPan-ITS Windows 安装器脚本

!include "MUI2.nsh"

; 基本信息
Name "XinPan-ITS"
OutFile "XinPan-ITS-Setup.exe"
InstallDir "$PROGRAMFILES64\XinPan-ITS"
...
Section "主程序" SecMain
    ...
    ; 创建快捷方式
    CreateDirectory "$SMPROGRAMS\XinPan-ITS"
    CreateShortcut "$SMPROGRAMS\XinPan-ITS\XinPan-ITS.lnk" "$INSTDIR\start.bat"
    CreateShortcut "$DESKTOP\XinPan-ITS.lnk" "$INSTDIR\start.bat"
    
    ; 写入卸载信息
    WriteUninstaller "$INSTDIR\Uninstall.exe"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\XinPan-ITS" \
                     "DisplayName" "XinPan-ITS"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\XinPan-ITS" \
...
Section "Uninstall"
    ...
    ; 删除快捷方式
    Delete "$DESKTOP\XinPan-ITS.lnk"
    RMDir /r "$SMPROGRAMS\XinPan-ITS"
    
    ; 删除注册表
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\XinPan-ITS"
SectionEnd
```
...
def build_installer():
    ...
    if result.returncode == 0:
        print("✅ 安装器构建成功！")
        print(f"📦 输出文件: XinPan-ITS-Setup.exe")
    else:
        print("❌ 安装器构建失败")
        print(result.stderr)
        sys.exit(1)
